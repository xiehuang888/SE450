...
final class InventorySet implements Inventory {
  ...
  Record checkOut(Video video) {
    ...
    if (!_data.containsKey(video) || record.numOwned() == record.numOut()){
      throw  new IllegalArgumentException("There is no record!");
    }else {
      Record newrecord = new RecordObj(video,record.numOwned(),record.numOut() + 1, record.numRentals() + 1);
      _data.replace(video,newrecord);
      assert newrecord.numOut() - record.numOut() == 1;
      assert newrecord.numRentals() - record.numRentals() == 1;
      return record;
    }
  }
  ...
  Record checkIn(Video video) {
    ...
    if (record == null || record.numOut() <= 0){
      ...
      Record newrecord = new RecordObj(video,record.numOwned(),record.numOut() - 1, record.numRentals());
      _data.replace(video,newrecord);
      assert newrecord.numOut() + 1 ==  record.numOut();
      return record;
    }
  }
  ...
  Map clear() {
    // TODO
    Map<Video,Record> toReturn = new HashMap<>();
    toReturn.putAll(_data);
    _data.clear();
    return toReturn;
  }
